...
type Config struct {
	...
	// YTDLP_PATH (and then to "yt-dlp") when unset.
	YtDlpBin string

	// FfmpegBin is the ffmpeg binary used by the music service. Falls back to
	// FFMPEG_PATH (and then to "ffmpeg") when unset.
	FfmpegBin string
	...
	// unset, only single tracks work — resolved through public oEmbed.
...
func Load() (*Config, error) {
	cfg := &Config{
		...
		MusicDownloadEnabled: getEnvBool("MUSIC_DOWNLOAD_ENABLED", true),
