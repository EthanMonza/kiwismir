...
// cobaltError is the JSON shape of the error field in a Cobalt response.
type cobaltError struct {
	Code string `json:"code"`
}

// CobaltError carries the error code returned by the Cobalt API, so callers
// can react to specific failure reasons (e.g. tweets locked behind a login).
type CobaltError struct {
	Code string
}

func (e *CobaltError) Error() string { return "cobalt error: " + e.Code }
...
func (d *Downloader) cobaltGetURL(ctx context.Context, rawURL, quality, mode string) (string, error) {
	...
	if cobResp.Error != nil {
		return "", &CobaltError{Code: cobResp.Error.Code}
	}
	...
	case "error":
		...
		return "", &CobaltError{Code: errCode}
	default:
