...
**A fast, friendly Telegram bot for downloading photos & videos from Pinterest, YouTube, TikTok, Instagram and Twitter/X.**
...
- 🐦 Download **videos from Twitter/X** — plain `x.com` / `twitter.com` links plus `vxtwitter` / `fxtwitter` / `fixupx` embed mirrors; age-restricted tweets unlock with `COOKIES_FILE`
...
- 🎵 **Music downloads**: `/yt <YouTube URL>` for audio; **Spotify tracks, albums and playlists** (pasted or via `/track`) are resolved to a YouTube search and sent back as `.mp3` — albums/playlists need `SPOTIFY_CLIENT_ID`/`SPOTIFY_CLIENT_SECRET` (full mode), tracks work without them; gated by a startup self-check and `MUSIC_DOWNLOAD_ENABLED`
...
│   ├── music/
│   │   ├── handlers.go          # /yt, /track and Spotify paste flows
...
| `/track`     | Download audio (`.mp3`) from a Spotify track, album or playlist URL.|
...
message unless `COOKIES_FILE` is configured.
...
| `MUSIC_DOWNLOAD_ENABLED` |        | `true`             | Toggle the music features (`/yt`, `/track`, Spotify pastes) |
| `YTDLP_BIN`            |          | `YTDLP_PATH`       | yt-dlp binary for the music service      |
| `FFMPEG_BIN`           |          | `FFMPEG_PATH`       | ffmpeg binary for the music service      |
| `COOKIES_FILE`         |          | —                  | Netscape cookies.txt for yt-dlp, applied to Twitter/X links only (age-restricted/sensitive tweets) |
